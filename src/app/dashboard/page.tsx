...
import { useExpensesSuapabase } from '@/hooks/use-expenses-supabase';
import { usePlatformsSuapabase } from '@/hooks/use-platforms-supabase';
...
export default function Dashboard() {
  ...
  const { 
    ...
    sortExpenses,
    sortOption,
  } = useExpensesSuapabase();
  
  const { platforms, addPlatform, updatePlatform, deletePlatform, loading: platformsLoading } = usePlatformsSuapabase();
  ...
  useEffect(() => {
    ...
    const hasPlatforms = localStorage.getItem(PLATFORMS_KEY);
    if (hasMaster || hasPlatforms) {
  ...
  const handleMigration = async () => {
    setIsMigrating(true);
    try {
      const { migrateLocalStorageToSupabase } = await import('@/lib/migrate-data');
      const { expensesMigrated, platformsMigrated, error } = await migrateLocalStorageToSupabase();
      ...
      if (error) {
        ...
        toast({
          ...
          description: `${expensesMigrated} pengeluaran dan ${platformsMigrated} platform telah dipindahkan ke database.`,
        });
          ...
          <Card className="mb-6 border-blue-200 bg-blue-50 shadow-sm">
            <CardContent className="p-4">
              <div className="flex flex-col gap-3">
                <div>
                  <h3 className="font-semibold text-foreground">Data Lokal Ditemukan</h3>
                  <p className="text-sm text-muted-foreground mt-1">Anda memiliki data yang tersimpan di browser. Ingin memindahkannya ke database untuk akses multi-perangkat?</p>
                </div>
                <div className="flex gap-2">
                  <Button 
                    ...
                    variant="default"
                  >
                    {isMigrating ? 'Memindahkan...' : 'Pindahkan Data'}
                  </Button>
                  <Button 
